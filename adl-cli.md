...
- The generated `.github/workflows/infer.yml` takes its model from the `INFER_MODEL` repository variable (`vars.INFER_MODEL`) - set it to a `provider/model` value such as `anthropic/claude-opus-4-8` and add the matching provider API key secret. When the variable is unset, [`infer-action`](/github-action/) applies its own default model; a `/model provider/name` override in the triggering issue or comment still wins for that run. The workflow also needs a GitHub App's client ID and private key in the `INFER_APP_ID` / `INFER_APP_PRIVATE_KEY` secrets (names configurable via `appIdSecret` / `appPrivateKeySecret`).
