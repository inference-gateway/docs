...
1. **Never hardcode API keys.** Store them in GitHub Secrets and reference them via `${{ secrets.NAME }}`. The action only reads provider keys from `inputs`, which are injected as environment variables for the agent step.
