...
- **Screen Recording** - Record the screen, a window, or a region to MP4 from chat ([Learn more](/cli-screen-recording/))
...
`RecordStart` and `RecordStop` record the primary screen, a single window, or a region to an H.264 MP4 - an audit trail of an unattended run, or a tutorial clip. Off by default; enable with `computer_use.recording.enabled` in `computer_use.yaml`. Needs ffmpeg with libx264 on `PATH`. See [Screen Recording](/cli-screen-recording/) for the capture modes, configuration, approval behaviour, and the AG-UI `screen_recording` event.
...
| **Computer Use**      | Computer, GetLatestFrame, RecordStart, RecordStop                                  | Accessibility-tree reads, presses, screenshots, and pointer/keyboard control - see the Computer Use section above; screen recording is opt-in via `computer_use.recording.enabled`, see [Screen Recording](/cli-screen-recording/)                           |
