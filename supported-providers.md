...
description: Provider matrix for Inference Gateway covering OpenAI, Anthropic, Cohere, Groq, Cloudflare, Ollama, Ollama Cloud, Google, DeepSeek, Mistral, MiniMax, Moonshot, Nvidia and the audio-only ElevenLabs (speech, sound effects and avatar videos), with auth modes, default URLs, vision support, and per-provider request examples.
---
